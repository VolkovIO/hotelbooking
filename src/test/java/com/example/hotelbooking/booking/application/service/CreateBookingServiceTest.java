...
class CreateBookingServiceTest {
  ...
  void shouldNotPlaceInventoryHoldWhenGuestCountExceedsRoomCapacity() {
    ...
    when(inventoryLookupPort.hotelExists(hotelId)).thenReturn(true);
    when(inventoryLookupPort.roomTypeExists(hotelId, roomTypeId)).thenReturn(true);
    when(inventoryLookupPort.findRoomTypeGuestCapacity(hotelId, roomTypeId))

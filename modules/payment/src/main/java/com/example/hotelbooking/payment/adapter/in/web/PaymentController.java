...
class PaymentController {
  ...
  PaymentResponse authorize(@Valid @RequestBody AuthorizePaymentRequest request) {
    return PaymentResponse.from(
        authorizePaymentService.authorize(
            new AuthorizePaymentCommand(
                request.bookingId(), request.userId(), request.amount(), request.currency())));
  }

  @PostMapping("/{paymentId}/approve")
  PaymentResponse approve(@PathVariable UUID paymentId) {
    return PaymentResponse.from(
        approvePaymentService.approve(new ApprovePaymentCommand(paymentId)));
  }

  @PostMapping("/{paymentId}/cancel")
  PaymentResponse cancel(@PathVariable UUID paymentId) {
    return PaymentResponse.from(cancelPaymentService.cancel(new CancelPaymentCommand(paymentId)));
  }
